pipeline {
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/steamos") | .uuid' || :)
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/steamos",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-steamos",\
                       "backendUrl":"https://ghcr.io/linuxserver/steamos",\
                       "publicUrl":"https://lscr.io/linuxserver/steamos"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-steamos/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-steamos\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-steamos\" \
